...
public class WorldGenerator_Flat extends ChunkGenerator {
	...
	public byte[][] generateBlockSections(World world, Random random, int x, int z, BiomeGrid biomes) {
		...
		for (x =0; x < 16; x++) {
			for (z = 0; z < 16; z++) {
				...
				// Dirt for 3 levels
				setBlock(result, x, 4, z, (byte)Material.DIRT.getId());
				setBlock(result, x, 5, z, (byte)Material.DIRT.getId());
				setBlock(result, x, 6, z, (byte)Material.DIRT.getId());
				
				//Dirt for 1 level
				setBlock(result, x, 6, z, (byte)Material.GRASS.getId());
			}
		}
